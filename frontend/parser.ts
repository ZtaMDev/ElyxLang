...
export default class Parser {
    ...
    private expect(type: TokenType, err: any) {
        ...
        if (!prev || prev.type != type) {
            throw new Error("Parser Error: " + err + " token: " + JSON.stringify(prev) + " Expecting: " + type);
        }
    ...
    public produceAST (sourceCode: string): Program {
        ...
        while (this.not_eof()) {
            ...
            if (this.at().type != TokenType.EOF && this.at().type != TokenType.CloseBrace) {
                ...
                if (startsNewStmt) {
                    throw new Error("Parser Error: Multiple statements in one line without semicolon aren't allowed. Current token: " + JSON.stringify(this.at()) + " Next tokens: " + JSON.stringify(this.tokens.slice(0,6)));
                }
            }
        }
        return program;
